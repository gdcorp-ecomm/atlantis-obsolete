...
{
  public class SSLProduct : MyaProductBase, IMyaProductAutoRenewable, IMyaProductPaymentProfile
  {
    protected override string AccountExpirationDateKey { get { return "expireDate"; } }
    ...
    public override bool IsRenewable
    {
      get
      {
        if (_isRenewable == null)
        {
          if (IsPropertyInDictionary("allowRenewals"))
          {
            _isRenewable = Convert.ToBoolean(PropertiesDictionary["allowRenewals"]);
          }
        }
        else
        {
    ...
    public bool IsSetup
    {
      get
      {
        return IsPropertyInDictionary("expireDate");        
      }  
    }
    ...
    public int? PaymentProfileShopperProfileId
    {
      get
      {
        if (_paymentProfileShopperProfileId == null)
        {
          ...
          if (IsPropertyInDictionary("mya_getActiveUnifiedListHostingSSL_sp"))
          {
    ...
    public SSLProduct(int privateLabelId, IDictionary<string, object> propertiesDictionary)

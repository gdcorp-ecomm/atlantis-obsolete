...
namespace Atlantis.Framework.MyaProductCostcoBundle.Impl
{
  public class MyaProductCostcoBundleRequest : IRequest
  {
    ...
    private const string STORED_PROCEDURE = "dbo.mya_getActiveCostCoBundlesList_sp";
    ...
    private const string PRODUCT_TYPE_ID = "65";

    #endregion

    public IResponseData RequestHandler(RequestData requestData, ConfigElement config)
    {
      MyaProductCostcoBundleResponseData responseData = null;

      try
      {
        MyaProductCostcoBundleRequestData request = (MyaProductCostcoBundleRequestData)requestData;
        ...
        using (SqlConnection cn = new SqlConnection(Nimitz.NetConnect.LookupConnectInfo(config)))
        {
          using (SqlCommand cmd = new SqlCommand(STORED_PROCEDURE, cn))
          {
            ...
            IList<CostcoBundleProduct> costcoBundleProductList = new List<CostcoBundleProduct>();
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
              while (reader.Read())
              {
                CostcoBundleProduct costcoBundleProduct = GetCostcoBundleProduct(reader, request);
                if (costcoBundleProduct != null)
                {
                  costcoBundleProductList.Add(costcoBundleProduct);
                }
              }
            }
            ...
            responseData = new MyaProductCostcoBundleResponseData(costcoBundleProductList, totalRecords, totalPages);
          }
          cn.Close();
        }
      }
      ...
      {
        responseData = new MyaProductCostcoBundleResponseData(exAtlantis);
      }

      catch (Exception ex)
      {
        responseData = new MyaProductCostcoBundleResponseData(requestData, ex);
      }

      return responseData;
    }

    #region Populate Costco Bundle Product
    private CostcoBundleProduct GetCostcoBundleProduct(IDataReader reader, MyaProductCostcoBundleRequestData request)
    {
      CostcoBundleProduct costcoBundleProduct = null;
      ...
      if (reader.FieldCount > 0)
      {
        ...
        costcoBundleProduct = new CostcoBundleProduct(request.PrivateLabelId, productProperties);
      }

      return costcoBundleProduct;
    }
    #endregion

    #region SQL Parameter Handling
    private SqlParameter[] SetSqlParameters(MyaProductCostcoBundleRequestData request)
    {
    ...
    private string GetSortColumnValue(MyaProductCostcoBundleRequestData.SortColumnType sortColumnType)
    {
      string sortColumn = string.Empty;

      switch (sortColumnType)
      {
        case MyaProductCostcoBundleRequestData.SortColumnType.AccountExpirationDate:
          sortColumn = EXPIRATION_DATE_SORT_COLUMN;
          break;
        case MyaProductCostcoBundleRequestData.SortColumnType.CommonName:
